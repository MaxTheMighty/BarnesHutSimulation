...
impl BarnesHutRunner {
    ...
    pub fn from_theta(theta: body::Float) -> BarnesHutRunner {
        Self {
            theta,
            paused: false,
        }
    ...
    pub fn generate_square(
        ...
        bodies: &mut Vec<Body>,
        length: u16,
        x_top_left: body::Float,
        y_top_left: body::Float,
    ) {
        for x in 0..length {
            for y in 0..length {
    ...
    pub fn generate_circle(
        ...
        let mut x_pos: body::Float = 0.0;
        let mut y_pos: body::Float = 0.0;

        for ring in 1..radius as u16 {
            for i in 0..360 {
                x_pos = (radius
                    - ring as body::Float * f32::cos(i as f32 * std::f32::consts::PI / 180.0))
                    + x_center;
    ...
    pub fn generate_bivariate_random_dist(
        &mut self,
        bodies: &mut Vec<Body>,
        width: f32,
        height: f32,
    ...
    pub fn resize(&mut self, quadtree: &mut Quadtree, bodies: &mut Vec<Body>) {
        ...
        let mut smallest: body::Float = quadtree.boundaries.tl.x;
        let mut largest: body::Float = quadtree.boundaries.br.x;
        for body in bodies {
